<template>
	<div v-if="showNotice" class="oldversion-notice">
		...
		We no longer provide support or release updates for brawlup.js v0. <a href="/js/additional/v0tov1">Please update to v1.</a>
	</div>
</template>
...
export default {
	...
	computed: {
		showNotice() {
			return semver.satisfies(semver.coerce('0.x'), this.selectedBranch) && (!this.hideUntil || Date.now() > parseInt(this.hideUntil));
		},
	},
	mounted() {
